...
import './SignUp.css';
import { citizenRegister } from '../../utils/api';
...
const SignUp = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await citizenRegister(citizenData);
      if (response.ok) {
    ...
    <div style={{
      ...
      <Form className="formstyle" noValidate validated={validated} onSubmit={handleSubmit}>
        <div className='divhed'>
          <h1 className='formhead'>Create Account</h1>
        </div>
        {showSuccessAlert && (
        ...
        <Form.Group controlId="txtPassword" className='pd-3'>
        ...
        <Button className='mt-2 text-center' type="submit">Sign Up</Button>
      </Form>
    </div>
  );
};
