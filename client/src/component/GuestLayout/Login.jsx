...
import { citizenLogin } from '../../utils/api';
...
const Login = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    setLoginErrors({});
    try {
      const response = await citizenLogin(loginData);
      if (response.ok) {
        const result = await response.json();
        const student = result.citizen;

        cookies.set('citizenData',
          JSON.stringify(student), { path: '/' })

          console.log(cookies.get('citizenData'))

        navigate('/citizen'); // Redirect to admin layout
      ...
      <Container className='container-2'>
        ...
        <div className="form form-1">
          ...
          <Form onSubmit={handleLoginSubmit}>
            ...
            <Link to="/citizenForgotPassword" className="forgot-password d-block mb-3">
              Forgotten password?
            </Link>
            <hr />
