...
import { circleOfficerLogin } from '../../utils/api';
import { Link, useNavigate } from 'react-router-dom';
import cookies from 'js-cookie'

const CircleOfficerLogin = () => {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        setLoginErrors({});
        try {
            const response = await circleOfficerLogin(loginData);
            if (response.ok) {
                const result = await response.json();
                const student = result.citizen;

                cookies.set('circleOfficerData',
                    JSON.stringify(student), { path: '/' })

                console.log(cookies.get('circleOfficerData'))

                navigate('/circle'); // Redirect to admin layout
            ...
            <Container className='container-2'>
                ...
                <div className="form form-1">
                    ...
                    <Form onSubmit={handleLoginSubmit}>
                        ...
                        <Link to="/circleOfficerForgotPassword" className="forgot-password d-block mb-3">
                            Forgotten password?
                        </Link>
                    </Form>
                </div>
...
export default CircleOfficerLogin;
