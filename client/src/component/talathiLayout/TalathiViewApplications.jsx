...
const TalathiViewApplications = () => {
  ...
  const handleDigitalSign = async (institute) => {
    try {
  ...
  const handleReject = async (institute) => {
    try {
      const response = await rejectApplication(institute._id);
      if (response.ok) {
          ...
          {applications.map((application, index) => (
            <Row key={index} className="table-row">
              ...
              {application.approved === true || application.staffApprove == true &&
                <Col xs={12} sm={1} className="text-center">Approved</Col>
              }
              ...
              <Col xs={12} sm={2} className="text-center">
                <Button variant="success" onClick={() => handleDigitalSign(application)}>Digital Sign</Button>
              </Col>
...
export default TalathiViewApplications
