...
import { approveApplication, circleApproveApplication, circleRejectApplication, getAllApplications } from '../../utils/api';
import { Button, Col, Container, Form, Modal, Row } from 'react-bootstrap';

const CircleViewApplication = () => {
    ...
    const getApplications = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const filteredApplications = data.filter(application => application.staffApprove && !application.circleApprove && !application.circleRejected);
    ...
    const handleApprove = async (institute) => {
        try {
            const response = await circleApproveApplication(institute._id);
            if (response.ok) {
    ...
    const handleReject = async (institute) => {
        try {
            const response = await circleRejectApplication(institute._id);
            if (response.ok) {
                alert("Status updated successfully");
                getApplications();
                handleClose();
            ...
            <Container className="container-1">
                <div className="list-container scrollable-list">
                    ...
                    <Row className="table-header pb-2 pt-2 GuestHeader">
                        ...
                        <Col xs={12} sm={3} className="text-center">Applicant Name</Col>
                        <Col xs={12} sm={2} className="text-center">Village</Col>
                        <Col xs={12} sm={1} className="text-center">Status</Col>
                    ...
                    {applications.map((application, index) => (
                        <Row key={index} className="table-row">
                            ...
                            <Col xs={12} sm={3} className="text-center">{application.applicantName}</Col>
                            <Col xs={12} sm={2} className="text-center">{application.village}</Col>

                            {application.rejected === true &&
                                <Col xs={12} sm={1} className="text-center">Rejected</Col>
                            }

                            {application.approved !== true && application.staffApprove == true &&
                                <Col xs={12} sm={1} className="text-center">Approved By Staff</Col>
                            }
...
export default CircleViewApplication
